...
class SolitaireCanvas {
  ...
  initMouseControl() {
    ...
    if (isTouchDevice) {
      ...
      this.addCanvasEventListener('touchend', onTouchEnd, { passive: true });
      this.addCanvasEventListener('touchcancel', onTouchEnd, { passive: true });
    } else {
  ...
  addCanvasEventListener(eventName, listener, options) {
    this.canvasEventListeners.push({ eventName, listener });
    this.canvas.addEventListener(eventName, listener, options);
  }
